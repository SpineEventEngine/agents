...
and, if it fails, runs the `bump-version` skill and re-runs to confirm.
...
2. Interpret the exit code:
   ...
   - **2** — Configuration error (no merge-base, parse failure on
     `version.gradle.kts`). Do **not** run the `bump-version` skill
     automatically. Surface the script's stderr to the user and stop.

3. On exit 1, run the `bump-version` skill to perform the actual bump. That
