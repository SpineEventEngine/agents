...
- **JUnit 5 gives the structure; Kotest provides the assertions.** We do not
  ...
  styles or test engine) while there is plenty of work on the Spine SDK
  itself.
...
- **Annotate every suite with `@DisplayName`**, backticking the name of the
  ...
  prefer backticked names over `@DisplayName`, put them on a new line, and
  start them with a capital letter.
- **Separate reusable fixtures from specs** using the Gradle
  [Test Fixtures plugin](https://docs.gradle.org/current/userguide/java_testing.html#sec:java_test_fixtures).
