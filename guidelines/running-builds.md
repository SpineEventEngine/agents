...
3. Documentation-only changes in Kotlin or Java sources run:
   ...
   CI's Dokka run does. (In a project that applies only the base Dokka
...
5. When code changes touch KDoc/Javadoc (any `.kt`/`.java` edit can rename or
   ...
   addition to `build`. The `build` task does **not** run Dokka, so a broken
   doc link passes `./gradlew build` locally yet fails CI's Dokka run — the PR
   build runs `dokkaGenerate`, with the publish job as a backstop. Combine them
   in one invocation, e.g. `./gradlew build dokkaGenerate`.
