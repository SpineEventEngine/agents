...
Some skills edit or stamp files across a whole repository — `proofread`
rewrites prose, `update-copyright` refreshes headers. Both must touch only
the files the repository **owns**. Editing an upstream file is worse than a
no-op: the next `./config/pull` (or submodule update) overwrites the change
or turns it into a merge conflict, and the same edit is needed again.

This page defines "project-owned" and the two kinds of upstream-owned paths
to skip. Each consuming skill applies the rule to the file types it
processes — the shared mechanism is here; the per-skill file filter is in
that skill.

## What to skip

A repository does **not** own two kinds of tracked files. Skip both in
every mode (a full sweep, a scoped sweep, and an incremental branch-diff
run alike).
...
- **`buildSrc/`** — the entire Gradle build-logic tree (hundreds of Kotlin
  ...
  **Exception:** `buildSrc/src/main/kotlin/module.gradle.kts` is
  ...
  overwrites it. Only that exact file; its `*-module.gradle.kts` siblings
  (for example `jvm-module.gradle.kts`) *are* distributed and stay skipped.
...
The `config` and `agents` repositories declare no `config` submodule, so
rule 2 is inert there: their own `AGENTS.md`, `buildSrc/`,
`gradle.properties`, and the rest are project-owned and stay in scope. This
is correct — a fix or a stamp must originate at the source that floats to
every consumer.
