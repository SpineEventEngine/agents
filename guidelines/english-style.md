...
TSDoc/JSDoc, Go doc comments, other code comments, and Markdown
documentation. Layout and flow rules (line wrapping, widows and orphans,
link style) live in [Documentation & comments](documentation.md); this page
covers the language itself.

The catalog below is the single source of truth for three kinds of
consumers:

- **Fixers** — the `proofread` skill edits existing text, applying a fix
  only when no leave-alone guard matches.
- **Reviewers** — the `review-docs` skill flags catalog violations on
  changed lines.
- **Authors** — the writing skills (`writer`, `java-to-kotlin`) follow the
  same rules when producing new prose.
...
3. **When uncertain, leave it and report it.** A missed error is cheaper
   than a wrong fix. Ambiguous occurrences are recorded for a human to
   decide, not edited.
4. **Never touch code or machine-read text.** See the per-language list
   below.
...
|-------------------------|--------------------------------------------------------------------------------|
| Kotlin / Java           | KDoc and Javadoc body text and tag descriptions; block and line comments       |
| Protobuf                | `//` doc comments of types and fields, and file headers ([rules](protobuf.md)) |
| TypeScript / JavaScript | TSDoc/JSDoc body text and tag descriptions; block and line comments            |
| Go                      | Doc comments (`//` immediately above a declaration) and other comments         |
...
Some comment content is read by compilers and tools, or must stay verbatim
for other reasons. A "fix" applied there can change build behaviour or
break tooling. Skip:
...
- License and copyright headers — they are generated from the copyright
  profile and owned by the `update-copyright` skill.
- Quoted text (quotations stay verbatim), URLs, email addresses, version
  strings, and file paths.
- The machine-read prefix of TODO comments
  (see [TODO comments](todo-comments.md)); the description after the
  prefix is ordinary prose.
...
- The `Deprecated:` marker word, which tooling parses; the explanation
  after it is prose.
...
frequent in text by Russian speakers, whose relative pronoun *который*
covers both senses.
...
- Interrogative or determiner use, direct or embedded: "Which plugin…?",
  "decide which plugin", "depending on which mode", "no matter which",
  "which of the following".
- Sentence-initial "Which".
- A hyphen abuts it — kebab-case names such as `which-fixer` are
  identifiers, not pronouns.
- Fixed phrases: the fused relative "that which" and the idiom
  "which is which".
...
- Deliberately telegraphic contexts: headings, table cells, and
  changelog-style list fragments. Articles are enforced only in full
  sentences.
...
- Bare identifiers used as names: "Calls `close` after use" needs no
  article.
...
- "a number of X are …" and "the number of X is …" — different subjects,
  both correct.
...
- Go's leading identifier (see **Never edit**); fix only the verb that
  follows it.
- Imperative mood where it is the local convention: step-by-step
  instructions, tutorials, README commands, commit messages, CLI help
  text.
- Type summaries written as noun phrases ("A thread-safe cache of …") or
  with "Represents …" — both correct.
- "This method returns …" — wordy but not an error; do not trim.
...
- "It is recommended to use …" — correct; the infinitive complements the
  passive "recommended".
...
which/that topic owns that decision; "e.g." and "i.e." with or without a
following comma (dialect-linked).
...
- A number with a unit symbol ("a 5 GiB limit") — no hyphen by
  convention.
...
- "e.g." (for example) vs. "i.e." (that is), and "affect" vs. "effect",
  change meaning — fix only when the context makes the intent
  unambiguous; otherwise report.
...
- Missing apostrophes in contractions are spelling errors: "dont" →
  "don't".
...
For words where American and British English differ, **the unit of
consistency is the file**:
